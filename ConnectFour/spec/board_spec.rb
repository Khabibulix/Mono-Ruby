...
describe do
  ...
  specify 'Hash for col should return a key for the col, and values for cells in col' do
    ...
    expect(@board.populate_hash(test_col_hash, 'col',
                                'X').key?(1) && @board.populate_hash(test_col_hash, 'col', 'X').value?(6))
  end
  ...
  specify 'Hash for row should return a key for the row, and values for cells in row' do
    ...
    @board.add(1, 'X')
    test_col_hash = Hash.new { |h, k| h[k] = [] }
    expect(@board.populate_hash(test_col_hash, 'row',
                                'X').key?(6) && @board.populate_hash(test_col_hash, 'col', 'X').value?(1))
  end
